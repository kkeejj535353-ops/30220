...
# 페이지 기본 설정
st.set_page_config(page_title="무료 브롤스타즈 승률 조회기", page_icon="🥊", layout="centered")

st.title("🥊 무료 브롤스타즈 승률 조회기")
st.caption("API 키 없이 태그만 입력하면 최근 일반전 및 경쟁전 승률을 바로 계산합니다.")

# 플레이어 태그 입력창
player_tag = st.text_input("플레이어 태그 입력 (예: #2R20PL0UR 또는 2R20PL0UR)", value="")

def get_brawl_data_public(tag):
    # 태그 정리 (# 제거 및 대문자 변환)
    clean_tag = tag.strip().upper().replace("#", "")
    
    # 오픈 API 엔드포인트
    player_url = f"https://api.brawlify.com/v1/player/{clean_tag}"
    ...
    try:
        ...
        if response.status_code == 404:
            return None, "플레이어를 찾을 수 없습니다. 태그를 올바르게 입력했는지 확인해 주세요."
        ...
        return None, "요청 시간이 초과되었습니다. 잠시 후 다시 시도해 주세요."
    except Exception as e:
        return None, f"데이터를 가져오는 중 오류가 발생했습니다: {str(e)}"
...
if st.button("승률 즉시 조회"):
    if not player_tag:
        ...
        with st.spinner("전적 데이터를 불러오는 중..."):
            data, error = get_brawl_data_public(player_tag)
            
            if error:
                st.error(error)
            else:
                # 기본 프로필 정보
                st.subheader(f"👤 {data.get('name', 'Unknown')} 님의 최근 통계")
                
                col1, col2, col3 = st.columns(3)
                col1.metric("현재 트로피", f"{data.get('trophies', 0):,}개")
                col2.metric("최고 트로피", f"{data.get('highestTrophies', 0):,}개")
                col3.metric("3v3 승리 수", f"{data.get('3vs3Victories', 0):,}회")
                ...
                for b in battles:
                    ...
                    # 경쟁전 여부 판단 (soloRanked, teamRanked, ranked 모드)
